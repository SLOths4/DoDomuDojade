                ...
                <?php foreach ($modules as $module): ?>
                    <tr x-data="{ isActive : <?= json_encode($module->isActive) ?> }">
                        ...
                        <td class="px-4 py-2 border space-x-2">
                            <span x-bind:class="isActive ? 'bg-green-500' : 'bg-red-500'" class="px-2 py-1 rounded"><?= $module->isActive ? "Włączony" : "Wyłączony" ?></span>
                            <form method="POST" action="/panel/toggle_module" style="display: inline;">
                                <input type="hidden" name="module_id" value="<?= e((string)$module->id) ?>">
                                <input type="hidden" name="csrf_token" value="<?= $csrf_token ?>">
                                <button type="submit"
                                        class="toggle-btn px-2 py-1 rounded text-white font-bold"
                                        :class="isActive ? 'bg-red-500 hover:bg-red-600' : 'bg-green-500 hover:bg-green-600'">
                                    <span x-text="isActive ? 'Wyłącz' : 'Włącz'"></span>
                                </button>
                            </form>
                        </td>
                        <td class="px-4 py-2 border space-x-2">
