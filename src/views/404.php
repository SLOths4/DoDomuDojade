<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="utf-8">
    <title>DoDomuDojadę | 404 error</title>
    <link rel="icon" type="image/x-icon" href="/assets/resources/favicon.ico">
    <link rel="preconnect" href="https://fonts.googleapis.com">
    <link rel="preconnect" href="https://fonts.gstatic.com" crossorigin>
    <link href="https://fonts.googleapis.com/css2?family=Lato:ital,wght@0,100;0,300;0,400;0,700;0,900;1,100;1,300;1,400;1,700;1,900&display=swap" rel="stylesheet">
    <link href="/assets/styles/output.css" rel="stylesheet" type="text/css">
    <script src="https://kit.fontawesome.com/d85f6b75e6.js" crossorigin="anonymous"></script>
    <script src="https://code.jquery.com/jquery-3.6.0.min.js"></script>
</head>
<body class="bg-white dark:bg-gray-900 dark:text-white">
<section class="bg-white dark:bg-gray-900">
    <div class="py-8 px-4 mx-auto max-w-screen-xl lg:py-16 lg:px-6">
        <div class="mx-auto max-w-screen-sm text-center">
            <h1 class="mb-4 text-7xl tracking-tight font-extrabold lg:text-9xl text-primary-400 dark:text-primary-300">404</h1>
            <p class="mb-4 text-3xl tracking-tight font-bold text-gray-800 md:text-4xl dark:text-gray-200">
                Resource not found.
            </p>
            <p class="mb-4 text-lg font-light text-gray-600 dark:text-gray-400">
                Tutaj nawet Kombus nie dojeżdża.
            </p>
            <a href="/"
               class="inline-flex text-white bg-secondary-200 hover:bg-secondary-100
                          focus:ring-4 focus:outline-none focus:ring-secondary-300
                          font-medium rounded-lg text-sm px-5 py-2.5 text-center
                          dark:focus:ring-secondary-900 my-4">
                Back to Homepage
            </a>
        </div>
    </div>
</section>
</body>
